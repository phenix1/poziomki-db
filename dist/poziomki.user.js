// ==UserScript==
// @name         Poziomki DB v2.5
// @namespace    https://poziomki.info
// @version      2.5
// @description  Recumbent bikes database (Deep Stealth, Hardcoded Themes, CSP Immune)
...
(function () {
  ...
  let COLLAB = {};
  let DB = [];
  let CONFIG = { version: "2.5" };
  let ADS = {}; 

  const SK = 'poziomki_state_v2_5';
  ...
  function getSeasonTheme() {
      ...
      if (month >= 2 && month <= 4) return { g1: '#1b4d3e', g2: '#2a7c63', thBg: '#eefcf5', thTxt: '#1b4d3e' }; // Wiosna
      if (month >= 5 && month <= 7) return { g1: '#0f5b78', g2: '#1ba1d3', thBg: '#eef8fc', thTxt: '#0f5b78' }; // Lato
      if (month >= 8 && month <= 10) return { g1: '#5c3a21', g2: '#a46533', thBg: '#fdf7f2', thTxt: '#5c3a21' }; // Jesień
      return { g1: '#162b45', g2: '#2a6090', thBg: '#eef3fa', thTxt: '#162b45' }; // Zima
  }
          ...
          GM_xmlhttpRequest({
              ...
              onload: (res) => {
                  if (res.status >= 200 && res.status < 300) {
                      ...
                      } catch (e) { reject(new Error('Błąd JSON')); }
  ...
  function getStyles(theme) {
    return `
      ...
      .pzk-sys-card a:active { transform: translateY(1px); }
  ...
  function buildUI() {
    ...
    wrap.innerHTML = `
      ...
      <div id="pzk-mainframe">
        ...
        <div id="pzk-grid-wrapper">
          <table id="pzk-data-grid">
            <thead><tr>
              <th id="pzk-s-p" style="width:25%;">Producer</th>
              <th id="pzk-s-m" style="width:35%;">Model</th>
              <th id="pzk-s-t" style="width:15%;">Type</th>
              <th id="pzk-s-k" style="width:15%;">Max Load</th>
              <th style="width:10%;">Link</th>
            </tr></thead>
            <tbody id="pzk-data-body"></tbody>
          </table>
        </div>
  ...
  async function initApp() {
    ...
    try {
      ...
      loadingWrap.innerHTML = `<div id="pzk-titlebar"><span class="title">Poziomki DB Error</span></div><div class="error-msg"><strong>Błąd krytyczny bazy:</strong><br>${error.message}</div>`;
    }
  }
