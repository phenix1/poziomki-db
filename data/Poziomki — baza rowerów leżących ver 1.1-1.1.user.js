// ==UserScript==
// @name         Poziomki — baza rowerów leżących ver 1.1
// @namespace    https://poziomki.info
// @version      1.1
// @description  Kompletna baza (432 modele) — PRZYWRÓCONY STYL v1.1 Michała Berlińskiego.
...
(function () {
  ...
  const DB = [
    ...
    { p:'Cruzbike', m:'A4x Complete', type:'bike', kg:113, url:'https://cruzbike.com/' },
    { p:'Cruzbike', m:'Freerider', type:'bike', kg:113, arch:true, url:'https://cruzbike.com/' },
    { p:'Cruzbike', m:'Q45', type:'bike', kg:113, url:'https://cruzbike.com/' },
    { p:'Cruzbike', m:'Quest', type:'bike', kg:113, arch:true, url:'https://cruzbike.com/' },
    { p:'Cruzbike', m:'S40', type:'bike', kg:113, url:'https://cruzbike.com/' },
    { p:'Cruzbike', m:'Silvio', type:'bike', kg:113, arch:true, url:'https://cruzbike.com/' },
    { p:'Cruzbike', m:'Sofrider', type:'bike', kg:113, arch:true, url:'https://cruzbike.com/collections/all-products/bicycles-and-framesets' },
    { p:'Cruzbike', m:'T50', type:'bike', kg:113, url:'https://cruzbike.com/' },
    { p:'Cruzbike', m:'V20c', type:'bike', kg:113, url:'https://cruzbike.com/products/v20c' },
    { p:'Cruzbike', m:'Vendetta', type:'bike', kg:113, arch:true, url:'https://cruzbike.com/' },
    ...
    { p:'Matix Bike', m:'FAT', type:'tadpole', kg:160, url:'https://matixbike.eu/matix-fat/' },
    ...
    { p:'ReActive Adaptations', m:'Nuke Off-Road', type:'handcycle', kg:0, url:'https://reactiveadaptations.com/nuke-recumbent-offroad-handcycle/' },
    ...
    { p:'Trident', m:'Fat Trekker', type:'tadpole', kg:125, url:'https://tridenttrikes.com/jouta-delta/products/trekker/' },
    ...
    { p:'Trident', m:'Terrain 20', type:'tadpole', kg:125, url:'https://tridenttrikes.com/jouta-delta/products/terrain/' },
    { p:'Trident', m:'Terrain 26', type:'tadpole', kg:147, url:'https://tridenttrikes.com/jouta-delta/products/terrain/' },
    ...
    { p:'TrikExplor', m:'F420 4WD', type:'quad', kg:0, url:'https://www.trikexplor.com/pages/trikexplor-recumbent-quad' },
    { p:'TrikExplor', m:'Sport Utility Quad', type:'quad', kg:0, url:'https://www.trikexplor.com/pages/trikexplor-recumbent-quad' },
  ...
  // ============================================================
  // STAN I STYLE (ODZYSKANE Z v1.1)[cite: 10]
  // ============================================================
  ...
  GM_addStyle(`
    #pdb-wrap {
      ...
      border: 1.5px solid #c0cce0; border-radius: 12px;
      box-shadow: 0 8px 32px rgba(0,40,100,.14); overflow: hidden;
    ...
    #pdb-hdr .badge { background: rgba(255,255,255,.2); color: #fff; border-radius: 10px; padding: 1px 9px; font-size: 11px; font-weight: 700; }
    #pdb-body { overflow-y: auto; flex: 1; }
    #pdb-wrap.col #pdb-body { display: none; }
    .pdb-ctrl { padding: 8px 12px 6px; display: flex; flex-wrap: wrap; gap: 5px; background: #f4f7fb; border-bottom: 1px solid #e0e8f0; }
    .pdb-ctrl select, .pdb-ctrl input { font-size: 12px; padding: 4px 7px; border: 1px solid #c4d0e0; border-radius: 6px; background: #fff; flex: 1; min-width: 90px; outline: none; }
    #pdb-tbl { width: 100%; border-collapse: collapse; table-layout: fixed; }
    #pdb-tbl thead th { background: #eef3fa; font-size: 10px; font-weight: 700; color: #4a6080; text-transform: uppercase; padding: 6px 10px; text-align: left; border-bottom: 1px solid #dce8f4; cursor: pointer; position: sticky; top: 0; z-index: 2; }
    #pdb-tbl tbody tr.row-collab-yes td { background: #f0fdf4 !important; border-left: 3px solid #22c55e; }
    #pdb-tbl tbody tr.row-collab-closed td { background: #fff5f5 !important; border-left: 3px solid #f87171; }
    .pdb-type { display: inline-block; font-size: 10px; padding: 1px 6px; border-radius: 8px; font-weight: 600; white-space: nowrap; }
    ...
    .t-velomobile { background: #fff4e0; color: #996620; }
    .t-handcycle  { background: #e0faff; color: #206699; }
    .kg-200plus { color: #6a10a0; font-weight: 700; }
    .kg-150plus { color: #1a4494; font-weight: 700; }
    .kg-none { color: #aaa; font-style: italic; font-size: 11px; }
    .pdb-link a { display: inline-block; font-size: 11px; padding: 2px 8px; border: 1px solid #c0d0e4; border-radius: 5px; text-decoration: none; color: #2a6090; background: #f0f6ff; font-weight: 500; }
    .pdb-csv { display: inline-block; margin-left: 8px; font-size: 10px; padding: 1px 6px; border: 1px solid #c0d0e0; border-radius: 4px; cursor: pointer; color: #2a6090; background: #fff; text-decoration: none; }
    .pdb-foot { padding: 5px 12px; font-size: 10px; color: #aaa; text-align: center; border-top: 1px solid #e8eef4; background: #f8fafc; }
  ...
  const TYPE_CLASS = { tadpole:'t-tadpole', delta:'t-delta', bike:'t-bike', quad:'t-quad', velomobile:'t-velomobile', handcycle:'t-handcycle' };
  ...
  function getFiltered() {
    return DB.filter(r => {
      ...
      const av = col === 'kg' ? (a.kg || 0) : (a[col] || '').toString().toLowerCase();
      const bv = col === 'kg' ? (b.kg || 0) : (b[col] || '').toString().toLowerCase();
      if (col === 'kg') return state.sortDir * (av - bv);
      return state.sortDir * av.localeCompare(bv, 'pl', {numeric: true});
    });
  }
  ...
  function build() {
    ...
    wrap.innerHTML = `
      ...
      <div id="pdb-body">
        <div class="pdb-ctrl">
          ...
          <input type="number" id="pdb-kg" placeholder="Min. load (kg)" value="${state.minKg || ''}">
        </div>
        <table id="pdb-tbl">
          <thead>
            <tr>
              <th onclick="pdbSort('p')">Producer</th>
              <th onclick="pdbSort('m')">Model</th>
              <th onclick="pdbSort('type')">Typ</th>
              <th onclick="pdbSort('kg')">Load kg</th>
              <th>Link</th>
            </tr>
          </thead>
        ...
        <div class="pdb-foot">
          Data: 2025 · kg=0 = no data · <strong>MBFeniks</strong>
    ...
    document.getElementById('pdb-hdr').onclick = () => {
    ...
    ['pdb-prod', 'pdb-type', 'pdb-kg'].forEach(id => {
      document.getElementById(id).onchange = (e) => {
        if(id==='pdb-prod') state.filterProd = e.target.value;
        if(id==='pdb-type') state.filterType = e.target.value;
        if(id==='pdb-kg') state.minKg = parseInt(e.target.value) || 0;
        save(); render();
      };
    });
  ...
  function render() {
    ...
    tbody.innerHTML = rows.map(r => {
      ...
      <tr class="${rowClass}">
        <td><b>${r.p}</b></td>
        <td>${r.m} ${r.arch?'🏛️':''} ${r.check?'🔍':''}</td>
        <td><span class="pdb-type ${TYPE_CLASS[r.type]}">${TYPE_LABEL[r.type]}</span></td>
        <td><span class="${kgClass(r.kg)}">${r.kg ? r.kg+' kg' : 'no data'}</span></td>
